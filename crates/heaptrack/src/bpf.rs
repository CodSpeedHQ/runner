...
impl HeaptrackBpf {
    ...
    /// Start polling the event ring buffer
    ///
    /// Returns a RingBufferPoller that manages the polling thread
    /// Note: Both BPF programs share the same ring buffer (events)
    pub fn start_polling(
        &self,
        handler: crate::poller::EventHandler,
        poll_timeout_ms: u64,
    ) -> Result<RingBufferPoller> {
        // Use the syscalls skeleton's ring buffer (both programs share the same one)
        RingBufferPoller::new(&self.skel.maps.events, handler, poll_timeout_ms)
    }
