...
use clap::Parser;
use heaptrack::{Event, HeaptrackIpcMessage, Tracker, handle_ipc_message};
use ipc_channel::ipc::{self};
use log::{debug, info};
use runner_shared::benchmark_results::{BenchmarkResultExt, HeaptrackResult};
...
fn track_command(
    ...
    let ipc_handle = if let Some(server_name) = ipc_server_name {
        ...
        let (tx, rx) = ipc::channel::<HeaptrackIpcMessage>()?;
    ...
    Ok((root_pid as u32, events.into_inner()?))
}
