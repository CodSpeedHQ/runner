...
>>>>>>> d74ff24 (feat: parse perf file for memmap events instead of relying on /proc/pid/maps)
        ...
        let on_cmd = async |cmd: &FifoCommand| {
            ...
            match cmd {
                ...
                FifoCommand::CurrentBenchmark { pid, .. } => {
                    #[cfg(target_os = "linux")]
                                         if parse_memory_maps && !symbols_by_pid.contains_key(pid) && !unwind_data_by_pid.contains_key(pid) {
