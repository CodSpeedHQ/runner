...
pub struct ProjectConfig {
    ...
    // Note: benchmarks field will be added in future PR
}
...
/// Result of config discovery, including the config and its path
#[derive(Debug, Clone)]
pub struct ProjectConfigDiscovery {
    /// The loaded configuration
    pub config: ProjectConfig,
    /// The absolute path where the config was found
...
impl ProjectConfig {
    ...
    ///
    /// # Returns
    /// * `Ok(Some(discovery))` - Config found and loaded successfully
    /// * `Ok(None)` - No config file found (not an error)
    /// * `Err(_)` - Error loading or parsing config
    pub fn discover_and_load(
        config_path_override: Option<&Path>,
        current_dir: &Path,
    ) -> Result<Option<ProjectConfigDiscovery>> {
    ...
    fn validate_walltime_options(opts: &WalltimeOptions, context: &str) -> Result<()> {
        ...
        if opts.min_time.is_some() && opts.max_rounds.is_some() {
            bail!(
                "Invalid walltime configuration in {}: cannot use both min_time and max_rounds",
                context
            );
        }
        ...
        if opts.max_time.is_some() && opts.min_rounds.is_some() {
            bail!(
                "Invalid walltime configuration in {}: cannot use both max_time and min_rounds",
                context
            );
        }
...
"#,
        ...
        let discovery =
            ProjectConfig::discover_and_load(Some(&config_path), temp_dir.path()).unwrap();

        assert!(discovery.is_some());
        let discovery = discovery.unwrap();
        assert!(discovery.config.options.is_some());
    }
...
"#,
        ...
        let discovery = ProjectConfig::discover_and_load(None, temp_dir.path()).unwrap();
...
"#,
        ...
        assert!(discovery.is_some());
        let discovery = discovery.unwrap();
        // Should find .yaml first
        assert!(discovery.config_path.ends_with("codspeed.yaml"));
    }
