...
pub struct GenericFifo {
    ctl_path: PathBuf,
    ack_path: PathBuf,
    sender: TokioPipeSender,
    receiver: TokioPipeReader,
}
...
impl GenericFifo {
    pub fn new(ctl_fifo: &Path, ack_fifo: &Path) -> anyhow::Result<Self> {
        ...
        let sender = get_pipe_open_options().open_sender(ctl_fifo)?;
        let receiver = get_pipe_open_options().open_receiver(ack_fifo)?;
    ...
    pub fn ctl(&mut self) -> &mut TokioPipeSender {
        &mut self.sender
    }

    pub fn ack(&mut self) -> &mut TokioPipeReader {
        &mut self.receiver
    }
...
impl RunnerFifo {
    ...
    ) -> anyhow::Result<FifoBenchmarkData> {
