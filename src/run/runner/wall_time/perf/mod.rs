...
impl PerfRunner {
    ...
    async fn handle_fifo(
        ...
        // FIXME: Ensure we don't share this across tasks
        let perf_fifo = RefCell::new(perf_fifo);
        let mut perf_ping_timeout = 5;
        let health_check = async || {
            let perf_ping = tokio::time::timeout(
                Duration::from_secs(perf_ping_timeout),
                perf_fifo.borrow_mut().ping(),
            )
            .await;
        ...
        let on_cmd = async |cmd: &FifoCommand| {
            let mut perf_fifo = perf_fifo.borrow_mut();
            match cmd {
                FifoCommand::StartBenchmark => perf_fifo.start_events().await?,
                FifoCommand::StopBenchmark => perf_fifo.stop_events().await?,
